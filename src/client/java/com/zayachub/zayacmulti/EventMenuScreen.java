...
public class EventMenuScreen extends Screen {
    ...
    protected void init() {
        ...
        addToggleButton("Сундук Смерті", "смерти", x, y + step * 7);

        this.addDrawableChild(ButtonWidget.builder(Text.of("§6Зберегти"), button -> {
            // Зберігаємо конфіг
            ConfigManager.config.enabledEvents = new java.util.HashSet<>(EventsModLogic.enabledEvents);
            ConfigManager.save();

            this.close();
        }).dimensions(x, y + step * 8 + 10, 200, 20).build());
    }
