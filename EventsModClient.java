...
public class EventsModClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        configKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "key.zayacevents.menu",
                InputUtil.Type.KEYSYM,
                GLFW.GLFW_KEY_J,
                "category.zayachub.events"
        ));
        ...
        // Використовуємо лише один тип івенту для стабільності
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> handleEventMessage(message));
    ...
    public static void handleEventMessage(Text text) {
        ...
        // Швидка перевірка: якщо повідомлення порожнє або це повідомлення від самого моду, ігноруємо
        ...
        if (count == 3) {
            if (pendingEventName != null && (System.currentTimeMillis() - pendingEventTime) < 3000) {
                // Встановлюємо івент ТИХО (без зайвих повідомлень у чат)
    ...
    private static void logToChat(String text) {
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc.player != null) {
            // Використовуємо системне повідомлення, щоб не засмічувати лог чату
    ...
    public static class EventTarget {
        ...
        public EventTarget(String name, int x, int y, int z) { this.name = name; this.x = x; this.y = y; this.z = z; }
    }
}
